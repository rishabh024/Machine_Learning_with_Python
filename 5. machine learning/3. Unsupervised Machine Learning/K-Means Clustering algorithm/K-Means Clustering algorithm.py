...
plt.scatter(C_x, C_y, marker='*', s=300, c='r')                # centroid loc is printed
plt.show()
...
while error.all():      # error != 0
    ...
    for i in range(len(X)):          # len(X) : 3000
        distances = eu_dist(X[i], C)      # distances = [ 2 data visualization , 50 , 9 ]
